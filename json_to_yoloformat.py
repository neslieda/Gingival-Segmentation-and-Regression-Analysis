...
def draw_outline(image_path, coordinates, color=(0, 255, 0), thickness=2):
    # Load image
    image = opencvdeneme.imread(image_path)
    ...
    opencvdeneme.polylines(mask, [coordinates], isClosed=True, color=color, thickness=thickness)

    # Add the outline to the original image
    result = opencvdeneme.addWeighted(image, 1, mask, 1, 0)

    # Display the result
    opencvdeneme.imshow('Image with Outline', result)
    opencvdeneme.waitKey(0)
    opencvdeneme.destroyAllWindows()
...
if __name__ == "__main__":
    # Replace with your image and txt file paths
    image_path = "dental/dental/gazi_gum_project/local/images/Makesens.ai - Yüksek Gülme Hattı-20240611T131617Z-001/Makesens.ai - Yüksek Gülme Hattı/1-3/1/IMG_3591..jpg"
