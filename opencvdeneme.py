...
def draw_lines_and_save(img_path, output_path):
    ...
    # Orta noktanın doğru belirlendiğinden emin olmak için kırmızı bir çizgi çizelim
    img_with_middle_point = img.copy()
    draw_middle = ImageDraw.Draw(img_with_middle_point)
    draw_middle.line((middle_point, 0, middle_point, height), fill="red", width=1)
    img_with_middle_point.save("middle_point_check.bmp")
    ...
    for point in points_of_interest_right:
        ...
        draw.line((new_point, 0, new_point, height), fill="blue", width=1)
